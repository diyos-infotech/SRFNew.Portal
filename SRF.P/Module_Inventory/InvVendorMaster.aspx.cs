    ...
    public partial class InvVendorMaster : System.Web.UI.Page
    {
        ...
        public void GetGridData()
        {
            try
            {
                ...
                string qry = "select itemid,itemname,BuyingPrice,case  VatCmp1  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp1per + "')/100) end VatCmp1," +
               " case  VATCmp2  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp2per + "')/100) end VATCmp2,case  VATCmp3  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp3per + "')/100) end VATCmp3, " +
                " case  VATCmp4  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp4per + "')/100) end VATCmp4,case  VATCmp5  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp5per + "')/100) end VATCmp5,(BuyingPrice+(case  VatCmp1  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp1per + "')/100) end)+case  VATCmp2  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp2per + "')/100) end +case  VATCmp3  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp3per + "')/100) end+case  VATCmp4  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp4per + "')/100) end+case  VATCmp5  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp5per + "')/100) end) as Total from InvStockItemList ";
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string insertquery = string.Format("insert into InvVendorMaster(VendorID,VendorName,Address,ContactPerson,ContactNo,Remarks,Created_By,Created_On,EMailId" +
                " ) values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')",
                  VendorId, VendorName, address, contactperson, contactnos, remarks, UserID, Created_On, emailid);
        ...
        {
            ItemIdAuto();
            txtVendorName.Text = txtAddress.Text = txtContactPerson.Text = txtContactNos.Text = txtRemarks.Text = txtemailid.Text = string.Empty;
            GVItemList.DataSource = null;
            GVItemList.DataBind();
        }
