...
namespace SRF.P.Module_Inventory
{
    public partial class ResourceReturnEmp : System.Web.UI.Page
    {
                ...
                foreach (GridViewRow gvr in gvresources.Rows)
                {
                    ...
                    if (chkresource != null)
                    {
                        ...
                        if (chkresource.Checked == false)
                        {
                            ...
                            Qty1 = int.Parse(txtIssuedQty.Text);
                            ResourceID = lblresourceid.Text;
                            amount1 = float.Parse(tb.Text);
                            TotalPrice1 = amount1;
                            ResourceType = lbltype.Text;

                            TotalQty1 += (Qty1) * (TotalPrice1);
                ...
                else
                {
                    ...
                    //else if (sum > 0)
                    {
                        if (issuemode == 0)
                        {
                            ...
                            SqlqryForResourceAlloc = string.Format("update EmpLoanMaster set RefundAmnt = isnull(RefundAmnt,0)+" + TotalQty + ", LoanAmount = case when ( " + TotalQty1 + " - " + paidamt + ")<0 then 0 else ( " + TotalQty1 + " - " + paidamt + ") end,PaidAmnt=case when ( " + TotalQty1 + " - " + paidamt + ")<0 then 0 else PaidAmnt end Where LoanNo = " + txtLoanno.Text + " and EmpId = '" + txtEmpid.Text + "'");
        ...
        protected void empLoadResourcedetails()
        {
            ...
            DataTable dtResources = config.ExecuteReaderWithQueryAsync(Sqlqry).Result;
