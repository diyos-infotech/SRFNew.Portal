...
namespace SRF.P.Module_Inventory
{
    public partial class ModifyInvVendorMaster : System.Web.UI.Page
    {
        ...
        public void GetData()
        {
            string qry = "select vendorid,vendorname,ContactPerson,ContactNo,Address,Remarks,EMailId from InvVendorMaster where vendorid='" + ddlVendorID.SelectedValue + "'";
        ...
        public void GetGridDataintoTable()
        {
            ...
            try
            {
                ...
                string sqry = "select isil.itemname,ivim.ItemId,ivim.BuyingPrice," +
                                ...
                                " union " +
                                " select itemname,itemid,BuyingPrice, " +
                                ...
                                " (BuyingPrice+(case  VatCmp1  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp1per + "')/100) end )+(case  VatCmp2  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp2per + "')/100) end)+(case  VatCmp3  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp3per + "')/100) end)+(case  VatCmp4  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp4per + "')/100) end)+(case  VatCmp5  when cast(0 as bit) then 0 else (BuyingPrice*('" + VATCmp5per + "')/100) end)) as Total,'','N' as Type from InvStockItemList where itemid not in (select ItemId from invvendoritemmaster where VendorID='" + ddlVendorID.SelectedValue + "') order by type,ItemName ";

                DataTable dt = config.ExecuteReaderWithQueryAsync(sqry).Result;
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataTable dtvendorname = config.ExecuteReaderWithQueryAsync(Selvendorname).Result;
                ...
                string qry = "update InvVendorMaster set Address='" + address + "',ContactPerson='" + contactperson + "',ContactNo='" + contactnos + "',Remarks='" + remarks + "',emailid='" + emailid + "',modified_by='" + UserID + "',modified_on='" + Modified_On + "' where vendorid='" + ddlVendorID.SelectedValue + "'";
        ...
        {
            txtAddress.Text = txtContactPerson.Text = txtContactNos.Text = txtRemarks.Text = txtemailid.Text = string.Empty;
        ...
        protected void lblBuyingPrice_OnTextChanged(object sender, EventArgs e)
        {
            ...
            lblTotal.Text = (float.Parse(lblBuyingPricev.Text) + float.Parse(lblVATCmp1.Text) + float.Parse(lblVATCmp2.Text) + float.Parse(lblVATCmp3.Text) + float.Parse(lblVATCmp4.Text) + float.Parse(lblVATCmp5.Text)).ToString("#.##");
            ...
            if (e.Row.RowType == DataControlRowType.Header || e.Row.RowType == DataControlRowType.DataRow)
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (dt.Rows[10]["Visibility"].ToString() == "Y")
                    {
                        ...
                        {
                            e.Row.Cells[4].Text = dt.Rows[10]["TaxCmpName"].ToString();
                        }
                    }
                    else
                    {
                    ...
                    if (dt.Rows[11]["Visibility"].ToString() == "Y")
                    {
                        ...
                        if (e.Row.RowType == DataControlRowType.Header)
                        {
                            e.Row.Cells[5].Text = dt.Rows[11]["TaxCmpName"].ToString();
                        }
                    }
                    else
                    {
                    ...
                    if (dt.Rows[12]["Visibility"].ToString() == "Y")
                    {
                        ...
                        if (e.Row.RowType == DataControlRowType.Header)
                        {
                            e.Row.Cells[6].Text = dt.Rows[12]["TaxCmpName"].ToString();
                        }
                    }
                    else
                    {
                    ...
                    if (dt.Rows[13]["Visibility"].ToString() == "Y")
                    {
                        ...
                        if (e.Row.RowType == DataControlRowType.Header)
                        {
                            e.Row.Cells[7].Text = dt.Rows[13]["TaxCmpName"].ToString();
                        }
                    }
                    else
                    {
                    ...
                    if (dt.Rows[14]["Visibility"].ToString() == "Y")
                    {
                        ...
                        if (e.Row.RowType == DataControlRowType.Header)
                        {
                            e.Row.Cells[8].Text = dt.Rows[14]["TaxCmpName"].ToString();
                        }
                    }
                    else
                    {
