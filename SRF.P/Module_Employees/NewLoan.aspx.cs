...
namespace SRF.P.Module_Employees
{
    public partial class NewLoan : System.Web.UI.Page
    {
        ...
        {
            string Sqlqry = "select (empfname+' '+empmname+' '+emplname) as empname,EmpDesgn from empdetails where empid='" + empid + "' and empid like '%" + EmpIDPrefix + "%'  ";
        ...
        {
            #region  Old Code
            string Sqlqry = "select empid Empid from empdetails where (empfname like '" + empname + "%' or  empmname like '" + empname + "%' or emplname like '" + empname + "%') and  empid like '%" + EmpIDPrefix + "%'  ";
            DataTable dt = config.ExecuteAdaptorAsyncWithQueryParams(Sqlqry).Result;
        ...
        private void GetLoanData()
        {
            string selectquery = "select LoanNo as LoanId,LoanAmount,lt.loantype as TypeOfLoan,case elm.LoanType when '' then '' else 'Remarks : '+ elm.LoanType end as remarks, " +
               ...
               txtEmpid.Text + "' ORDER BY LoanDt DESC";
