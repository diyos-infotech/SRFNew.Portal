...
{
    public partial class UnitwiseESIPFPTReports : System.Web.UI.Page
    {
        //DataTable dt;
        string EmpIDPrefix = "";
        string CmpIDPrefix = "";
            ...
            try
            {
                ...
                if (ddlClientId.SelectedIndex == 1)
                {
                    sqlqry = " select ed.EmpId,c.clientname,eps.clientid,rtrim(ISNULL(ed.EmpFName, '') + ' ' + ISNULL(ed.empmname, '') + ' ' + ISNULL(ed.emplname, '')) as Fullname, (round(eps.PFWAGES, 0)) as PFWAGES,case when DATEDIFF(year, EmpDtofBirth, GETDATE())< 58 then (round(eps.PFWAGES, 0)) else '0' end EPSWAGES," +
                             "  (round(eps.ESIonDuties, 0)) as ESIonDuties,(round(eps.ProfTax, 0)) as ProfTax,(round(eps.ESIWAGES, 0)) as ESIWAGES,(round(eps.ESI, 0)) as ESI,(round(eps.ESIONOT, 0)) as ESIONOT,(round(eps.ESIONOT*100/1.75, 0)) as ESIonOTWages,((round(eps.ESIONOT*100/1.75, 0))+(round(eps.ESIWAGES, 0))) as EmpConsESIWages, " +
