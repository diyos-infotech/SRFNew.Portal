...
namespace SRF.P.Module_Reports
{
    public partial class ModificationLoanReport : System.Web.UI.Page
    {
        ...
        protected void GetEmpid()
        {
            ...
            #region  Old Code
            string Sqlqry = "select Empid +' - '+ OldEmpid as Empid from empdetails where (empfname+' '+empmname+' '+emplname)  like '" + txtName.Text + "'";
        ...
        {
            string Sqlqry = "select ISNULL(ED.EmpId,'') as EmpId,ISNULL(ED.EmpFName,'')+' '+ISNULL(ED.EmpMName,'')+' '+ISNULL(ED.EmpLName,'') as empname,EmpDesgn from empdetails ED where empid= '" + txtEmpid.Text + "' ";
            DataTable dt = config.ExecuteAdaptorAsyncWithQueryParams(Sqlqry).Result;
            if (dt.Rows.Count > 0)
            {
                try
